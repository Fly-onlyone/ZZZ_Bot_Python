...
- **Dedup key:** `locator:<md5(selector)[:12]>` — one MongoDB document per unique selector
...
- **Storage:** `locator_tracker` collection with 7-day TTL index
...
MongoDB collections are backed up and restored via the `/backup/export` and `/backup/import` endpoints. The backup
system in `MongoRepository.py` categorizes collections into two types:

- **Single-doc collections:** `settings`, `account`, `shopping` — stored as one document each
- **Multi-doc collections:** `missions`, `redemptions`, `locator_tracker` — stored as arrays of documents

All collections are listed in `_SINGLE_DOC_COLLECTIONS` and `_MULTI_DOC_COLLECTIONS`. When adding a new MongoDB
collection, add its name to the appropriate set so it is included in backup export/import and the backup summary page.

**Key files:**

- `backend/repositories/MongoRepository.py` — `export_data()`, `import_data()`, collection sets

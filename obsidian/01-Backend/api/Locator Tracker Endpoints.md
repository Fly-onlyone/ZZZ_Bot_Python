...
- `GET /locator-tracker/child-scan/{summary_id}` returns the enriched DOM context (`child_scan` array) for a single summary entry by reading the `locator_tracker` collection directly.
...
- [[Mongo Connection]] — direct read for child scan
